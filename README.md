...
将「箱体突破四条件」超短线战法以 **Rust 原生重构**，搭配 **PostgreSQL (PgSQL)** 实现结构化数据存储与历史回溯，同时提供完整的图形化 Web 看板与实时逐股流水日志。
...
- 🐘 **PostgreSQL 结构化持久化与极致优化**：
  - **自动初始化建表**（Auto-migration）：启动时自动创建表结构与索引，无需手动执行 DDL。
  - **UNNEST 批量插入**：采用 PostgreSQL 原生数组展开批处理技术，240 根日 K 线秒级批量持久化入库，网络吞吐提升数百倍。
...
- 🌐 **双市场支持**：
  - **A 股市场**：全市场 5000+ 股票清单获取，东财 push2 + 腾讯行情 qt + 新浪金融多源主备容灾。
  - **加密货币**：Binance USDT 永续合约 24h 涨幅榜与日 K 线行情。
- 📜 **实时逐股同步与流水日志**：
  - 同步每只标的时实时输出：`[序号/总数] 股票代码 名称 现价 240根K线/资金/户数入库状态 评分 模式`。
  - 前端内置**实时流式日志控制台**，支持一键折叠与自动滚动。
...
# 2. 执行单次命令行扫描
cargo run --release -- --scan market    # 全市场 5000+ 股票扫描
...
| `klines` | 240 根日 K 线历史持久化 | `code`, `market`, `k_date`, `open`, `high`, `low`, `close`, `volume` |
...
| `POST` | `/api/pool` | 动态添加/移除自选标的 (`{"action":"add","code":"600519"}`) |
| `GET` | `/api/quotes?codes=...` | 批量实时行情（价格/涨跌/量比/换手） |
| `GET` | `/api/kline?code=...` | 获取日 K 线及 60 日箱体区间数据 |
| `GET/POST`| `/api/scan?mode=market` | 触发后台扫描（可选 `market` 全市场 / `quick` 活跃榜 / `pool` 自选 / `crypto` 币圈） |
| `GET` | `/api/status` | 获取扫描状态、逐股实时日志及开盘交易状态 |
...
├── start.sh            # 一键启动脚本
├── dashboard.html      # 前端交互看板页面（含蜡烛图与实时流式日志控制台）
├── static/             # 自托管字体与静态资源
│   └── fonts/
├── data/               # 本地数据文件与降级缓存
│   └── pool.json       # 自选股票池文件
└── src/                # Rust 原生核心源代码
    ...
    ├── models.rs       # 核心实体结构体定义
    ├── db.rs           # PostgreSQL 连接池、UNNEST 批量插入与自动建表
    ├── crawler.rs      # 多源行情爬虫（东财/腾讯/新浪/Binance）
    ├── engine.rs       # 60日箱体计算、试盘算法与打分引擎
    ├── server.rs       # Axum Web 路由、API 与实时日志管理
